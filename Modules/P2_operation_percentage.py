...
def return_boaring_rates(current_time, days_interval):
    ...
    temp_route_df = temp_route_df[['originDeptTime_datetime', 'destDeptTime_datetime', 'vehicleType', 'onboardingNum']]
    temp_route_df['Capacity'] = [max_seats[temp_route_df['vehicleType'][i]] for i in range(len(temp_route_df))]

    temp_route_df['trip_duration'] = (temp_route_df['destDeptTime_datetime'] - temp_route_df['originDeptTime_datetime']).dt.total_seconds()
    ...
    daily_past_df = (
        past_df
        .groupby(['Day', 'vehicleType'])
        .agg(
            total_duration=('trip_duration', 'sum'),
            boarded_duration=('trip_duration', lambda x: x[past_df.loc[x.index, 'boarded']].sum())
        )
        .reset_index()
    )
    daily_past_df['boarding_rate'] = daily_past_df['boarded_duration'] / daily_past_df['total_duration'] * 100

    daily_past_pivot = daily_past_df.pivot(index='Day', columns='vehicleType', values='boarding_rate')
    ...
    daily_last_df = (
        ...
        .agg(
            total_duration=('trip_duration', 'sum'),
            boarded_duration=('trip_duration', lambda x: x[last_df.loc[x.index, 'boarded']].sum())
        )
        .reset_index()
    )

    daily_last_df['boarding_rate'] = daily_last_df['boarded_duration'] / daily_last_df['total_duration'] * 100

    daily_last_pivot = daily_last_df.pivot(index='Day', columns='vehicleType', values='boarding_rate')
    ...
    hourly_df['boarding_rate'] = hourly_df['boarded_duration'] / hourly_df['total_duration'] * 100

    hourly_pivot = hourly_df.pivot(index='Hour', columns='vehicleType', values='boarding_rate')
    hourly_pivot = hourly_pivot.reindex(columns=vehicle_types)
    ...
    hourly_ma_carnivalReg = compute_centered_moving_average(hourly_df, '카니발(일반)', hourly_target_days, window_size)
    hourly_ma_carnivalWheel = compute_centered_moving_average(hourly_df, '카니발(휠체어)', hourly_target_days, window_size)
    hourly_ma_IONIQ5 = compute_centered_moving_average(hourly_df, '아이오닉5', hourly_target_days, window_size)
    ...
    chart_daily = generating_chart(daily_last_df, ma_carnivalReg, ma_carnivalWheel, ma_IONIQ5, '#006ACD', '#FFB1B0', '#7CBEF2', '실차 탑승시간 비율', '%', 'Day')
    chart_hourly = generating_chart(hourly_df, hourly_ma_carnivalReg, hourly_ma_carnivalWheel, hourly_ma_IONIQ5, '#006ACD', '#FFB1B0', '#7CBEF2', '실차 탑승시간 비율', '%', 'Hour')
